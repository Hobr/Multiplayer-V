...
{
	Vector3 v = GRAPHICS::GET_TEXTURE_RESOLUTION((char*)m_dict.c_str(), (char*)m_name.c_str());
...
{
	return m_dict != "" && m_name != "" && GRAPHICS::HAS_STREAMED_TEXTURE_DICT_LOADED((char*)m_dict.c_str());
}

void UITexture::EnsureLoaded() const
{
	if (!GRAPHICS::HAS_STREAMED_TEXTURE_DICT_LOADED((char*)m_dict.c_str())) {
		GRAPHICS::REQUEST_STREAMED_TEXTURE_DICT((char*)m_dict.c_str(), true);
	}
}
...
void UITexture::Render(const glm::vec2 &pos, const glm::vec2 &size, float heading, const glm::ivec4 &color) const
{
	...
	GRAPHICS::DRAW_SPRITE((char*)m_dict.c_str(), (char*)m_name.c_str(), spos.x, spos.y, ssize.x, ssize.y, heading, color.r, color.g, color.b, color.a);
}
